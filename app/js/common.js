const DELAY = 5,
  STP = 273,
  ELEMENT_MODAL_DATA = elementDescription;
...
function createModalContent(elementName){
  Object
    .keys(ELEMENT_MODAL_DATA[elementName])
    .forEach(key =>
      modalProperties.innerHTML += `<p class='element-property element-${key}'>${ELEMENT_MODAL_DATA[elementName][key]}</p>`
    );
}
